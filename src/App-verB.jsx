...
import { auth, db } from './firebase'; // <-- ต้องมี db
import { 
...
} from 'firebase/firestore'; // <-- ต้องมี Firestore Functions
...
// ==========================================
// 1. LOGIN MODAL (ใช้งาน Firebase จริง)
// ==========================================
const LoginModal = ({ onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isRegistering) {
                ...
                // เมื่อ Register สำเร็จ -> สร้าง Doc ใน Firestore
                const userRef = doc(db, "users", auth.currentUser.uid);
                await setDoc(userRef, { credits: 50, joinedAt: new Date() }); // แจก 50 เครดิต
            } else {
                await signInWithEmailAndPassword(auth, email, password);
            }
            onClose(); // ปิดหน้าต่างเมื่อสำเร็จ
        } catch (err) {
            console.error(err);
            if (err.code === 'auth/invalid-credential') setError('อีเมลหรือรหัสผ่านไม่ถูกต้อง');
            else if (err.code === 'auth/email-already-in-use') setError('อีเมลนี้มีคนใช้แล้ว');
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            // เช็คว่า User ใหม่หรือไม่
            const userRef = doc(db, "users", result.user.uid);
            const userSnap = await getDoc(userRef);
            
            if (!userSnap.exists()) {
                 // ถ้า User ใหม่ -> แจก 50 เครดิต
                 await setDoc(userRef, { credits: 50, joinedAt: new Date(), displayName: result.user.displayName });
            }
            
            onClose();
        } catch (err) {
            console.error("Google Login Error:", err);
            setError("ไม่สามารถเข้าสู่ระบบด้วย Google ได้ (อาจเป็นปัญหาเครือข่าย)");
        }
    };
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-slate-100">
                ...
                <div className="p-8 pt-10">
                    <div className="text-center mb-6">
                        ...
                        <h2 className="text-2xl font-black text-slate-900 mb-1">{isRegistering ? 'สร้างบัญชีใหม่' : 'ยินดีต้อนรับกลับมา!'}</h2>
                        <p className="text-slate-500 text-sm">{isRegistering ? 'สมัครสมาชิกเพื่อเริ่มใช้งานฟรี' : 'เข้าสู่ระบบเพื่อจัดการงานของคุณ'}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="relative"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Mail size={18} /></div><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all text-slate-900 font-medium" placeholder="name@example.com" required /></div>
                        <div className="relative"><div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"><Lock size={18} /></div><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all text-slate-900 font-medium" placeholder="••••••••" required /></div>
                        <button type="submit" disabled={isLoading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-all flex items-center justify-center gap-2 mt-2 active:scale-[0.98]">
                            {isLoading ? <Sparkles className="animate-spin" size={20} /> : (isRegistering ? <>สมัครสมาชิก <UserPlus size={18} /></> : <>เข้าสู่ระบบ <LogIn size={18} /></>)}
                        </button>
                    </form>

                    <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div><div className="relative flex justify-center text-xs"><span className="px-2 bg-white text-slate-400 font-medium">หรือดำเนินการต่อด้วย</span></div></div>
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-100 text-center">
                        <p className="text-xs text-slate-500">
                            {isRegistering ? 'มีบัญชีอยู่แล้ว?' : 'ยังไม่มีบัญชีใช่ไหม?'}
                            <button onClick={() => { setIsRegistering(!isRegistering); setError(''); }} className="text-indigo-600 font-bold hover:underline ml-1">
                                {isRegistering ? 'เข้าสู่ระบบเลย' : 'สมัครสมาชิกใหม่'}
                            </button>
                        </p>
                    </div>
                </div>
    ...
    <div className="min-h-screen bg-white font-sans text-slate-900 selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden relative">
      ...
      <main className="max-w-6xl mx-auto px-6 pt-16 pb-32 text-center relative z-10">
        ...
        <div className="flex flex-col items-center gap-2 mb-10">
            <h1 className="text-3xl md:text-6xl font-black text-slate-900 tracking-tight leading-[1.4] py-1">ปฏิวัติการทำ Content Marketing ของคุณ</h1>
            <h1 className="text-3xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 tracking-tight leading-[1.4] py-1">ด้วย AI ที่เน้น Hook แข็งแกร่ง</h1>
            <h1 className="text-3xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 tracking-tight leading-[1.4] py-1">และ Call to Action ที่ชัดเจน</h1>
        </div>

        <p className="text-lg md:text-xl text-slate-500 mb-10 max-w-3xl mx-auto leading-relaxed font-medium">ยกระดับขีดความสามารถในการแข่งขัน ด้วยแพลตฟอร์ม <span className="text-slate-900 font-bold">SekScript Pro</span> ผสานเทคโนโลยี Generative AI เข้ากับหลักการตลาดสากล เพื่อสร้างสคริปต์ที่แม่นยำ สื่อสารตรงจุด และสร้างผลลัพธ์ที่วัดผลได้จริง</p>
...
const ScriptGenerator = ({ onLogout }) => {
  ...
  const handleGenerate = async () => {
    ...
    if (credits < 5) { setShowTopUp(true); return; }
      ...
      {showTopUp && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm relative z-10 overflow-hidden p-6 text-center animate-in fade-in zoom-in-95">
                  ...
                  <p className="text-slate-500 text-sm mb-6">ใช้ 5 เครดิต ต่อการสร้าง 10 สคริปต์</p>
                  <button onClick={() => { setCredits(prev => prev + 50); setShowTopUp(false); alert("เติมเงินสำเร็จ! (จำลอง)"); }} className="w-full py-3 rounded-xl bg-yellow-400 text-slate-900 font-bold hover:bg-yellow-500 transition-all shadow-lg shadow-yellow-200 mb-3">เติม 50 เครดิต</button>
                        ...
                        <div className="p-6 space-y-6 bg-white">
                            ...
                            <div className="flex flex-wrap justify-between items-center gap-3 p-2 bg-slate-50 rounded-xl border border-slate-100">
                                <div className="flex bg-white rounded-lg p-1 border border-slate-200 shadow-sm">
                                    <button onClick={() => setGlobalLang('th')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${globalLang === 'th' ? 'bg-black text-white shadow' : 'text-slate-400 hover:text-slate-600'}`}>TH</button>
                                    <button onClick={() => setGlobalLang('en')} className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all ${globalLang === 'en' ? 'bg-black text-white shadow' : 'text-slate-400 hover:text-slate-600'}`}>EN</button>
                                </div>
...
// ==========================================
// 4. ROOT APP (Final Cleaned Management)
// ==========================================
const App = () => {
    ...
    <>
        ...
        {!isLoggedIn ? (
            <LandingPage 
                ...
                onQuickAccess={() => setIsLoggedIn(true)} // ปุ่ม Quick Access
            />
        ) : (
